...
//    system("ps aux | grep eulerSNP; date");
    if(argc < 2)
    {
      printf(" %s <-rlength 1000> <input fastq> <output fastq>\n",argv[0]);
      exit(1);
    }
...
//     char **rdname;
     ...
     int n_patch,idd,stopflag,num_gcs,n_contigs,*contig_index;
     float rate;
     fasta *segg,*seqp;
