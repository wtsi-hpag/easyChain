 ...
 *                                                                          *
 *  This file is part of easyChain  pipeline.                                 *
 *                                                                          *
...
int main(int argc, char **argv)
{
    ...
    char tempa[2000],tempc[2000],syscmd[2000],file_target[2000],file_refere[2000],file_easyChain[2000],workdir[2000];
    ...
    for(i=1;i<argc;i++)
    {
       ...
       else if(!strcmp(argv[i],"-shred"))
       {
         sscanf(argv[++i],"%d",&shred_len);
         args=args+2;
       }
    }
...
//    system (syscmd);
    ...
    sprintf(bindir,"%s/easyChain-bin",tempc);


    memset(file_target,'\0',2000);
    memset(file_refere,'\0',2000);
    memset(file_easyChain,'\0',2000);
    sprintf(file_target,"%s/%s",tempa,argv[args]);
    sprintf(file_refere,"%s/%s",tempa,argv[args+1]);
    sprintf(file_easyChain,"%s/%s",tempa,argv[args+2]);

    if((namef = fopen(file_target,"r")) == NULL)
    {
      ...
      {
        memset(file_target,'\0',2000);
        strcpy(file_target,argv[args]);
        printf("Input target file: %s\n",file_target);
      }
    }
    else
    {
    ...
    if((namef = fopen(file_refere,"r")) == NULL)
    {
      printf("File not in the working directory or the used command not recognised!\n");
      ...
      {
        memset(file_refere,'\0',2000);
        strcpy(file_refere,argv[args+1]);
        printf("Input refere file: %s\n",file_refere);
      }
    }
    else
    {
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/easyChain_fasta -name target -len 10 %s target.fasta target.tag > try.out",bindir,file_target);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/easyChain_fasta -name refere -len %d %s refere.fasta refere.tag > try.out",bindir,min_len,file_refere);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/easyChain_shred -name target -length %d target.fasta target.shred > try.out",bindir,shred_len);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/smalt index -k 13 -s 13 hash_refere refere.fasta  > try.out",bindir);
    if(system(syscmd) == -1)
    {
//  printf("System command error:\n);
    }
